function Header({ isMini }: { isMini: boolean }) {
  ...
  const appearDatesFrame = isMini ? 'opacity-100 scale-100' : ''
    ...
    >
      ...
      <div className={`flex relative transition-all duration-700 ${datesPosition}`}>
        <div className="absolute b-0 w-full h-full flex justify-center">
          <div
            className={`w-11/12 h-full rounded-lg bg-white bg-opacity-10 backdrop-blur-md scale-50 opacity-0 transition-all duration-500 ${appearDatesFrame}`}
          />
        </div>
